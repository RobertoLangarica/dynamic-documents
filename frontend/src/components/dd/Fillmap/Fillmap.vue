<template>
    <div class="column" style="position:relative;">
        <div class="col row justify-between content">
            <!-- SOURCE -->
            <div class="col-5">
        ...
        <div class="col-auto row justify-end q-pa-md q-col-gutter-x-xl">
            ...
            <div><q-btn color="primary" label="Guardar cambios" @click="onSaveChanges"/></div>
        </div>
...
export default class Fillmap extends Vue {
    ...
    onRemoveChange(from:DDField){
        ...
        let index = this.changes.fields.findIndex(item=> item.destination === fromID)
        if(index >= 0){
    ...
    onChange(from:DDField,to:DDField){
        ...
        // Delete any change on the destination
        let index = this.changes.fields.findIndex(item=> item.destination === toID)
        if(index >= 0){
            this.changes.fields.splice(index,1)
        }
