  ...
  methods: {
    ...
    async onAccept () {
      ...
      this.manager.fields.forEach(field => {
        if (this.group_id && field.group_by !== this.group_id) {
          // only valid group fields
          return
        } else if (this.field_id && field.id !== this.field_id) {
          // only valid fields
          return
        }

        let fmap_id = field.map_id || field.id
        let dest = this.fillmap.fields.find(map => fmap_id === map.destination || fmap_id === map.foreign)
        if (!dest) {
          // This field is not on the map
          return
        }

        let sourceField = this.selectedSource.fields.find(s => s.id === dest.destination || s.id === dest.foreign)
        if (!sourceField) {
          // The provided source has no value for the map
          return
        }

        // A valid change
        this.manager.updateField({ id: field.id, value: sourceField.value });
      })
      ...
      /* this.fillmap.fields.forEach(map => {
        ...
        if (dest && this.group_id && dest.group_by !== this.group_id) {
          // only valid group fields
          console.log('rejected for group', dest)
          dest = null
        } else if (dest && this.field_id && dest.id !== this.field_id) {
          console.log('rejected for field', dest)
          // only valid fields
          dest = null
        }
