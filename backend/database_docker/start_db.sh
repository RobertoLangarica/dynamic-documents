...
file=.env.example
exist_credentials=1
echo "Using credentias FROM: .env.example"
else
echo "Using credentias FROM: .env.example"
exist_credentials=0
fi
...
# Run the potgres server only if it is not running
msg="PstgresSQL already running. Use the next command to stop it: docker rm $(docker stop db_server)"
docker exec db_server echo $msg
...
# to avoid that we force the user and db initilization again
